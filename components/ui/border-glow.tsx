...
import { useCallback, useEffect, useRef, type ReactNode, type CSSProperties, type PointerEvent as ReactPointerEvent } from "react"
...
interface BorderGlowProps {
  ...
  colors?: string[]
  fillOpacity?: number
  /** Keeps the glow visibly on at all times instead of only near the
   * pointer - used for marking demo/mock profiles, where the border needs
   * to read as a persistent label, not a hover flourish. */
  persistent?: boolean
}

function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  const match = hslStr.match(/([\d.]+)\s*([\d.]+)%?\s*([\d.]+)%?/)
  if (!match) return { h: 0, s: 85, l: 55 }
...
export function BorderGlow({
  ...
  return (
    <div
      ...
      className={`border-glow-card${lightSurface ? " border-glow-card--light" : ""}${persistent ? " border-glow-card--persistent" : ""} ${className}`}
      style={
        {
