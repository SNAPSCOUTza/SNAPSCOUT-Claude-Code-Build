...
DEVICE_W = 1030
BEZEL = 11
SCREEN_W = DEVICE_W - 2 * BEZEL    # 1008
SCREEN_CORNER_R = 68
...
FONT_PATH = os.environ.get(
    "ASO_FONT_PATH",
    "/Library/Fonts/SF-Pro-Display-Black.otf" if os.name != "nt" else r"C:\Windows\Fonts\arialbd.ttf",
)
...
def compose(bg_hex, verb, desc, screenshot_path, output_path):
    ...
    # Device normally sits at a fixed Y, but if the headline wraps to enough
    # lines that it would overlap the device (e.g. a long description on a
    # narrow-verb layout), push the device down so there's always at least
    # MIN_TEXT_DEVICE_GAP between the text block and the device frame.
    text_top = 200
    device_y = max(DEVICE_Y, text_top + text_height + MIN_TEXT_DEVICE_GAP)
